...
from charms.tool.utils import cached, flush, Config, charm_dir, atexit
...
def relation_id(relation_name=None, service_or_unit=None):
    ...
    if not relation_name and not service_or_unit:
        ...
        raise ValueError('Must specify neither or both of relation_name and '
                         'service_or_unit')
...
def remote_service_name(relid=None):
    """The remote service name for a given relation-id, or the current rel)"""
    if relid is None:
        unit = remote_unit()
    else:
...
def peer_relation_id():
    '''Get the peers relid if a peers relation has been joined, or None.'''
    md = metadata()
...
def relation_to_role_and_interface(relation_name):
    ...
    that relation uses (where role is one of ``provides``, ``requires``,
    or ``peers``).
    ...
    for role in ('provides', 'requires', 'peers'):
        interface = _m.get(role, {}).get(relation_name, {}).get('interface')
        if interface:
...
def payload_status_set(klass, pid, status):
    ...
    registered with juju using payload-register. The <status> must be one of
    the follow: starting, started, stopping, stopped"""
